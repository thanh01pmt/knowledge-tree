...
def build_call_c(source_text: str, goal: str, features_summary: str, arch_summary: str, max_chars: int = 60000,
    ...
    system = BASE_SYSTEM + (
        ...
        "task_dependencies (REQUIRED, type per edge): distinguish the two dependency kinds —\n"
        "  - LEARNING_DEPENDENCY: pedagogical prerequisite — the learner must master task Y "
        "BEFORE task X (component before the form that uses it, model before the logic that "
        "uses it, mock/static UI before the real service). This is the KIND the roadmap follows.\n"
        "  - CODE_DEPENDENCY: compile-time/structural only — X references Y's code to build "
        "(WelcomeView NavigationLink targets LoginView; ChatDetail sends push via "
        "PushNotificationManager) but the learner CAN build X first with placeholders/mock data.\n"
        "Prefer LEARNING_DEPENDENCY whenever the dependency is a genuine knowledge prerequisite; "
        "a dependency that merely makes code compile is CODE_DEPENDENCY.\n"
        "Return JSON matching the schema — fill only the requested domains; leave the others empty.\n"
        + profile_extra + "\n"
    )
    user = (
