...
some_meta_services = [
    "packages/learnwell-platform/src/lib/services/questionBankService.ts",
    "packages/learnwell-platform/src/lib/services/classroomService.ts",
    "packages/learnwell-platform/src/lib/services/resourceService.ts",
    "packages/learnwell-platform/src/lib/actions/metadataActions.ts",
]

auth_related_services = [
    # ======================================================================
    # 1. Services: Lớp trung gian gọi các hàm RPC từ Supabase.
    # Đây là nơi logic gọi RPC được định nghĩa và là nơi có khả năng cao nhất
    # thiếu tham số `organization_code`.
    # ======================================================================
    "packages/learnwell-platform/src/lib/services/metadataService.ts",
    "packages/learnwell-platform/src/lib/services/questionBankService.ts",
    "packages/learnwell-platform/src/lib/services/classroomService.ts",
    "packages/learnwell-platform/src/lib/services/resourceService.ts",
    "packages/learnwell-platform/src/lib/actions/metadataActions.ts",
    ...
    # ======================================================================
    # 2. UI Components (Managers): Các component giao diện người dùng,
    # nơi người dùng thực hiện hành động (nhấn nút "Save", "Create"),
    # và là nơi gọi đến các hàm trong services ở trên.
    # ======================================================================
    ...
    "packages/learnwell-platform/src/components/features/metadata-iqk/CourseManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/LearningObjectiveManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/ApproachManager.tsx",
    "packages/learnwell-platform/src/components/features/authoring-iqk/authoring/QuestionFormDialog.tsx"
]


supabase_auth = [
    # Middleware & Route Protection
    "packages/learnwell-platform/src/middleware.ts",

    # Supabase Client Configuration
    "packages/learnwell-platform/src/lib/supabase/client.ts",
    "packages/learnwell-platform/src/lib/supabase/server.ts",

    # Authentication Flow Handlers
    "packages/learnwell-platform/src/app/auth/callback/route.ts",
    "packages/learnwell-platform/src/app/login/page.tsx",

    # State Management & UI Integration
    "packages/learnwell-platform/src/context/AuthContext.tsx",
    "packages/learnwell-platform/src/hooks/useAuth.ts",
    "packages/learnwell-platform/src/components/layout/UserSessionManager.tsx",
    "packages/learnwell-platform/src/app/(app)/layout.tsx",

    # Business Logic Services
    "packages/learnwell-platform/src/lib/services/authService.ts",
    "packages/learnwell-platform/src/lib/services/userService.ts",

    # Type Definitions
    "packages/learnwell-platform/src/types/entities/user.ts"
]

meta_managers = [
    # Các component quản lý metadata chính
    "packages/learnwell-platform/src/components/features/metadata-iqk/CategoryManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/SubjectManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/TopicManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/CourseManager.tsx",
    ...
    # Dialog tạo và sửa câu hỏi
    "packages/learnwell-platform/src/components/features/authoring-iqk/authoring/QuestionFormDialog.tsx",

    # Các dialog/component khác có thể tạo dữ liệu có organization_code
    # Mặc dù không có trong danh sách ban đầu, việc kiểm tra chúng là cần thiết
    "packages/learnwell-platform/src/components/admin/ExamTypeManager.tsx",
    "packages/learnwell-platform/src/components/teacher/class-details/ResourceManager.tsx",
    "packages/learnwell-platform/src/components/exams/ExamHubDashboard.tsx"
]

other_managers = [
    # Component quản lý Loại bài kiểm tra (Exam Types)
    # Cần kiểm tra logic onAdd/onUpdate để đảm bảo organization_code được truyền vào.
    "packages/learnwell-platform/src/components/admin/ExamTypeManager.tsx",

    # Component quản lý Tài nguyên học tập (Resources) trong lớp học
    # Cần xác minh rằng sub-component AddResourceDialog lấy organizationCode từ profile một cách chính xác khi gọi service.
    "packages/learnwell-platform/src/components/teacher/class-details/ResourceManager.tsx",

    # Dashboard trung tâm cho việc quản lý Bài kiểm tra
    # Cần kiểm tra luồng tạo mới Exam và Blueprint để đảm bảo organization_code được xử lý đúng.
    "packages/learnwell-platform/src/components/exams/ExamHubDashboard.tsx"
]

problem_files = [
    # =========================================================================
    # 1. Core Services, Actions, and Hooks (Nơi xử lý logic chính)
    # =========================================================================
    # Refactored to split responsibilities: client-side for sync, server-side for reads.
    "packages/learnwell-platform/src/lib/services/metadataService.client.ts",
    "packages/learnwell-platform/src/lib/services/metadataService.server.ts",

    # Server Actions to securely perform CUD operations. This is the core of the new architecture.
    "packages/learnwell-platform/src/lib/actions/genericMetadataActions.ts",
    "packages/learnwell-platform/src/lib/actions/metadataActions.ts",
    "packages/learnwell-platform/src/lib/actions/questionActions.ts",
    "packages/learnwell-platform/src/lib/actions/examBlueprintActions.ts",
    "packages/learnwell-platform/src/lib/actions/examActions.ts",
    "packages/learnwell-platform/src/lib/actions/assignmentActions.ts",

    # Client-side IndexedDB caching layer and hooks.
    "packages/learnwell-platform/src/lib/db/clientDB.ts",
    "packages/learnwell-platform/src/lib/hooks/useMetadata.ts",
    "packages/learnwell-platform/src/hooks/useMetadataImporter.ts",

    # Services for non-metadata entities that were part of the refactor.
    "packages/learnwell-platform/src/lib/services/questionBankService.ts",
    "packages/learnwell-platform/src/lib/services/examBlueprintService.ts",
    "packages/learnwell-platform/src/lib/services/examService.ts",
    "packages/learnwell-platform/src/lib/services/classroomService.ts",

    # =========================================================================
    # 2. Pages (Nơi người dùng tương tác trực tiếp)
    # =========================================================================
    # The main page for managing all metadata types.
    "packages/learnwell-platform/src/app/(app)/metadata/page.tsx",
    # The page for managing the question bank, which now uses the new architecture.
    "packages/learnwell-platform/src/app/(app)/questions/page.tsx",
    # The page for authoring exams and blueprints, also refactored.
    "packages/learnwell-platform/src/app/(app)/exams/page.tsx",
    # The teacher's class detail page, which contains the AssignmentManager.
    "packages/learnwell-platform/src/app/(app)/teacher/class/[classId]/page.tsx",

    # =========================================================================
    # 3. Core UI Components (Các component xây dựng nên các trang)
    # =========================================================================
    # All "Manager" components, now refactored to be self-sufficient.
    "packages/learnwell-platform/src/components/features/metadata-iqk/CourseManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/SubjectManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/CategoryManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/TopicManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/LearningObjectiveManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/ApproachManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/GradeLevelManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/BloomLevelManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/QuestionTypeManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/ContextManager.tsx",

    # Question authoring flow components that were refactored.
    "packages/learnwell-platform/src/components/questions/QuestionModals.tsx",
    "packages/learnwell-platform/src/components/features/authoring-iqk/authoring/EditQuestionModal.tsx",

    # Exam/Blueprint authoring flow components that were refactored.
    "packages/learnwell-platform/src/components/exams/ExamEditor.tsx",
    "packages/learnwell-platform/src/components/exams/ExamDetailsPanel.tsx",
    "packages/learnwell-platform/src/components/features/authoring-iqk/authoring/BlueprintCreationWizard.tsx",

    # Assignment authoring flow components that were refactored.
    "packages/learnwell-platform/src/components/teacher/class-details/AssignmentManager.tsx",
    "packages/learnwell-platform/src/components/teacher/class-details/AssignExamDialog.tsx",

    # Supporting metadata components.
    "packages/learnwell-platform/src/components/features/metadata-iqk/LearningObjectiveDialog.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/MetadataImportControls.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/MetadataImportErrorDialog.tsx",

    # =========================================================================
    # 4. Type Definitions (Định nghĩa cấu trúc dữ liệu)
    # =========================================================================
    # Files defining the data structures for metadata and content entities.
    "packages/learnwell-platform/src/types/entities/metadata.ts",
    "packages/learnwell-platform/src/types/entities/content.ts",
    "packages/learnwell-platform/src/types/entities/classroom.ts",
    "packages/learnwell-platform/src/types/entities/index.ts"
]
...
# ==============================================================================
# I. Lớp Hành động (Server Actions) - Cổng chính cho các thao tác Ghi/Sửa/Xóa
# Các file này được thiết kế để gọi từ Client Components và thực thi an toàn trên server.
# ==============================================================================
server_actions_files = [
    # Action chung, là wrapper bảo mật cho hầu hết các hành động metadata
    "packages/learnwell-platform/src/lib/actions/genericMetadataActions.ts",
    
    # Actions chuyên biệt cho các nghiệp vụ cụ thể
    "packages/learnwell-platform/src/lib/actions/adminActions.ts",
    "packages/learnwell-platform/src/lib/actions/assignmentActions.ts",
    "packages/learnwell-platform/src/lib/actions/classroomMemberActions.ts",
    "packages/learnwell-platform/src/lib/actions/examActions.ts",
    "packages/learnwell-platform/src/lib/actions/examBlueprintActions.ts",
    "packages/learnwell-platform/src/lib/actions/metadataActions.ts",
    "packages/learnwell-platform/src/lib/actions/questionActions.ts",
    "packages/learnwell-platform/src/lib/actions/questionBankActions.ts",
]

# ==============================================================================
# II. Lớp Dịch vụ (Services) - Chứa logic nghiệp vụ và các truy vấn Đọc/Ghi phức tạp
# Các service này được gọi bởi cả Server Components (để đọc dữ liệu) và Server Actions.
# ==============================================================================
service_layer_files = [
    # Dịch vụ cốt lõi cho các nghiệp vụ chính
    "packages/learnwell-platform/src/lib/services/authService.ts",
    "packages/learnwell-platform/src/lib/services/userService.ts",
    "packages/learnwell-platform/src/lib/services/classroomService.ts",
    "packages/learnwell-platform/src/lib/services/examService.ts",
    "packages/learnwell-platform/src/lib/services/examBlueprintService.ts",
    "packages/learnwell-platform/src/lib/services/examTypeService.ts",
    "packages/learnwell-platform/src/lib/services/questionBankService.ts",
    "packages/learnwell-platform/src/lib/services/resourceService.ts",
    "packages/learnwell-platform/src/lib/services/studentMasteryService.ts",
    "packages/learnwell-platform/src/lib/services/notificationService.ts",

    # Dịch vụ quản trị và báo cáo (chủ yếu là các truy vấn Đọc phức tạp)
    "packages/learnwell-platform/src/lib/services/adminService.ts",
    "packages/learnwell-platform/src/lib/services/teacherDashboardService.ts",
    "packages/learnwell-platform/src/lib/services/teacherAnalyticsService.ts",
    "packages/learnwell-platform/src/lib/services/reports/assetReportService.ts",
    "packages/learnwell-platform/src/lib/services/reports/examAnalyticsService.ts",
    "packages/learnwell-platform/src/lib/services/reports/studentProgressService.ts",
    "packages/learnwell-platform/src/lib/services/reports/systemOverviewService.ts",

    # Dịch vụ đồng bộ hóa dữ liệu offline/online
    "packages/learnwell-platform/src/lib/services/SyncService.ts",
    
    # Dịch vụ quản lý metadata (bao gồm cả client-side sync và server-side fetch)
    "packages/learnwell-platform/src/lib/services/metadataService.client.ts",
    "packages/learnwell-platform/src/lib/services/metadataService.server.ts",
]

# ==============================================================================
# III. Lớp Giao diện (UI Layer) - Các Server Component trực tiếp fetch dữ liệu
# Các file này thực hiện truy vấn Đọc khi render trang phía server.
# ==============================================================================
server_component_files = [
    # Một ví dụ điển hình về Server Component gọi nhiều service để lấy dữ liệu
    "packages/learnwell-platform/src/app/(app)/teacher/class/[classId]/page.tsx",
]

# ==============================================================================
# IV. Lớp Cấu hình & Kết nối (Configuration & Connection Layer)
# Các file này không trực tiếp thực hiện CRUD nhưng là nền tảng để các file trên hoạt động.
# ==============================================================================
connection_files = [
    # Client cho môi trường client-side
    "packages/learnwell-platform/src/lib/supabaseClient.ts",
    
    # Client cho môi trường server-side (Server Components, Actions, Route Handlers)
    "packages/learnwell-platform/src/lib/supabase/server.ts",
]

# ==============================================================================
# Tổng hợp tất cả các file liên quan
# ==============================================================================
all_related_files = sorted(list(set(
    server_actions_files +
    service_layer_files +
    server_component_files +
    connection_files
...
# ==============================================================================
# I. UI Components (Forms, Dialogs, Managers) - Chủ yếu gọi các Server Actions để Ghi/Sửa/Xóa
# Đây là các component tương tác, nơi người dùng thực hiện các hành động làm thay đổi dữ liệu.
# ==============================================================================
ui_component_consumers = [
    # Quản lý thành viên lớp học (approve, reject, add)
    "packages/learnwell-platform/src/components/teacher/class-details/MemberManager.tsx",
    
    # Quản lý bài tập (create, update, delete)
    "packages/learnwell-platform/src/components/teacher/class-details/AssignmentManager.tsx",
    "packages/learnwell-platform/src/components/teacher/class-details/AssignExamDialog.tsx",
    
    # Quản lý tài nguyên lớp học
    "packages/learnwell-platform/src/components/teacher/class-details/ResourceManager.tsx",
    
    # Quản lý chương trình học của lớp
    "packages/learnwell-platform/src/components/teacher/class-details/CurriculumManager.tsx",
    
    # Quản lý người dùng trong tổ chức (invite)
    "packages/learnwell-platform/src/components/admin/InviteUserDialog.tsx",
    "packages/learnwell-platform/src/app/(app)/admin/users/page.tsx", # Trang này cũng chứa logic gọi action
    
    # Các component quản lý metadata (Subjects, Topics, v.v.)
    "packages/learnwell-platform/src/components/features/metadata-iqk/CourseManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/SubjectManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/CategoryManager.tsx",
    "packages/learnwell-platform/src/components/features/metadata-iqk/TopicManager.tsx",
    ...
    # Các component liên quan đến soạn thảo đề thi và câu hỏi
    "packages/learnwell-platform/src/components/exams/ExamEditor.tsx",
    "packages/learnwell-platform/src/components/features/authoring-iqk/authoring/ExamCreationWizard.tsx",
    "packages/learnwell-platform/src/components/features/authoring-iqk/authoring/BlueprintCreationWizard.tsx",
    "packages/learnwell-platform/src/components/features/authoring-iqk/authoring/QuestionFormDialog.tsx",
    "packages/learnwell-platform/src/components/questions/QuestionModals.tsx",
    
    # Các component gọi đến AI services
    "packages/learnwell-platform/src/components/features/dashboard-iqk/AnalysisDialog.tsx",
    "packages/learnwell-platform/src/components/features/dashboard-iqk/Cheatsheet.tsx",
    "packages/learnwell-platform/src/components/teacher/class-details/ClassroomInsightsTab.tsx",
    "packages/learnwell-platform/src/components/features/settings-iqk/UploadResourceModal.tsx",
]

# ==============================================================================
# II. Data Display & Page Components - Chủ yếu gọi các Service để Đọc dữ liệu
# Đây là các Server Component hoặc Client Component chịu trách nhiệm hiển thị dữ liệu.
# ==============================================================================
data_display_and_page_consumers = [
    # Trang chi tiết lớp học (Server Component, fetch rất nhiều dữ liệu)
    "packages/learnwell-platform/src/app/(app)/teacher/class/[classId]/page.tsx",
    
    # Trang quản lý bài tập của Admin
    "packages/learnwell-platform/src/app/(app)/admin/assignments/page.tsx",

    # Trang quản lý người dùng của Admin
    "packages/learnwell-platform/src/app/(app)/admin/users/page.tsx",
    
    # Các trang báo cáo (gọi các report services)
    "packages/learnwell-platform/src/components/reports/system-overview/SystemOverviewReport.tsx",
    "packages/learnwell-platform/src/app/(app)/reports/asset-report/page.tsx",
    "packages/learnwell-platform/src/app/(app)/reports/exam-analytics/page.tsx",
    "packages/learnwell-platform/src/app/(app)/reports/student-performance/page.tsx",
    
    # Trang làm bài của học sinh (cần lấy QuizConfig)
    "packages/learnwell-platform/src/app/(app)/student/attempt/[assignmentId]/page.tsx",
    
    # Trang hồ sơ cá nhân
    "packages/learnwell-platform/src/app/(app)/profile/page.tsx",
]

# ==============================================================================
# III. Hooks & Context Providers - Lớp trung gian trừu tượng hóa việc gọi service
# Các file này không phải là UI nhưng chúng gọi services và cung cấp dữ liệu cho UI.
# ==============================================================================
hooks_and_context_consumers = [
    # Context trung tâm quản lý session và profile người dùng
    "packages/learnwell-platform/src/context/AuthContext.tsx",
    
    # Component headless chịu trách nhiệm đồng bộ dữ liệu khi người dùng đăng nhập
    "packages/learnwell-platform/src/components/layout/UserSessionManager.tsx",
    
    # Các custom hook để lấy và cache dữ liệu metadata từ IndexedDB
    "packages/learnwell-platform/src/lib/hooks/useMetadata.ts",
    
    # Custom hook để fetch và phân trang dữ liệu từ ngân hàng câu hỏi
    "packages/learnwell-platform/src/lib/hooks/questionBankHooks.ts",
]

# ==============================================================================
# IV. Tổng hợp tất cả các file "tiêu thụ"
# ==============================================================================
all_consuming_files = sorted(list(set(
    ui_component_consumers +
    data_display_and_page_consumers +
    hooks_and_context_consumers
)))
...
import_realted_files = [
    "src/app/(app)/questions/page.tsx",
    "src/components/questions/QuestionModals.tsx",
    "src/components/features/authoring-iqk/authoring/ImportQuestionsModal.tsx",
    "src/lib/services/questionBankService.ts",
    "src/lib/actions/questionActions.ts",
    "src/types/entities/content.ts"
]
...
live_session_files = [
    # ===================================================================
    # 1. DATABASE & EDGE FUNCTIONS (The Core Backend Logic)
    # ===================================================================

    # Edge Function called by the teacher's dashboard to get real-time updates.
    "supabase/functions/get-session-updates/index.ts",
    
    # Edge Function called by the student's client to post progress updates.
    ...
    # ===================================================================
    # 2. SERVER-SIDE APPLICATION LOGIC (Next.js Backend)
    # ===================================================================
    
    # Defines the server actions for creating, closing, joining, and submitting live sessions.
    "src/lib/actions/liveExamActions.ts",
    
    # The API route that acts as a proxy for the teacher's dashboard to call the Edge Function.
    "src/app/api/live-updates/route.ts",

    # The API route that acts as a proxy for the student's client to call the Edge Function.
    "src/app/api/live-progress/route.ts",


    # ===================================================================
    # 3. TEACHER-SIDE USER INTERFACE (Client & Server Components)
    # ===================================================================
    
    # The page where the teacher starts the flow.
    "src/app/(app)/exams/page.tsx",

    # Renders the list of exams and their action menus.
    "src/components/exams/ExamHubDashboard.tsx",

    # The individual card for an exam, containing the "Publish Live Session" button.
    "src/components/features/authoring-iqk/authoring/ExamCard.tsx",

    # The server component for the live dashboard page, responsible for initial data fetching.
    "src/app/(app)/teacher/live/[sessionId]/page.tsx",

    # The client component for the live dashboard, handles polling for updates.
    "src/components/teacher/live-session/LiveSessionDashboard.tsx",


    # ===================================================================
    # 4. STUDENT-SIDE USER INTERFACE (Client & Server Components)
    # ===================================================================

    # The public-facing server page that students access via the short code.
    "src/app/live/[shortCode]/page.tsx",
    
    # The client component that manages the entire student flow: joining, playing, and submitting.
    "src/app/live/[shortCode]/LiveExamClientFlow.tsx",
    
    # The core component that renders the quiz questions and handles user interaction.
    # We modified this to add the onProgressUpdate callback.
    # Note: The exact path may vary, this is based on our last modification.
    "src/lib/interactive-quiz-kit/react-ui/components/ui/QuizPlayer.tsx",


    # ===================================================================
    # 5. SHARED TYPES & UTILITIES (Used by all parts)
    # ===================================================================

    # Defines the TypeScript interfaces for LiveExamSession and LiveExamParticipant.
    "src/types/entities/classroom.ts",

    # Main Supabase client for client-side components.
    "src/lib/supabaseClient.ts",

    # Supabase client factory for server-side components and actions.
...
scorm_build_related_files = [
    # --- 1. File điều phối (Orchestration) ---
    "package.json",  # File ở gốc monorepo, định nghĩa script "build:scorm" để bắt đầu toàn bộ quá trình.

    # --- 2. File tự động hóa ---
    "sync_scorm_assets.py",  # Script Python để tự động sao chép các tài sản đã build sang platform.

    # --- 3. Cấu hình và script của package "interactive-quiz-kit" ---
    "packages/interactive-quiz-kit/package.json",  # Định nghĩa các script con "build:scorm:js" và "build:scorm:css".
    "packages/interactive-quiz-kit/tsup.scorm.config.ts",  # QUAN TRỌNG: Cấu hình build để tạo ra file player.js độc lập.
    "packages/interactive-quiz-kit/tailwind.config.ts",    # Cấu hình cho TailwindCSS để build file styles.css.
    "packages/interactive-quiz-kit/postcss.config.js",     # Cấu hình PostCSS, được Tailwind sử dụng.
    
    # --- 4. Mã nguồn đầu vào cho quá trình build ---
    "packages/interactive-quiz-kit/src/player.ts",          # Entry point chính cho mã nguồn JavaScript của SCORM player.
    "packages/interactive-quiz-kit/src/styles/main.css",    # File CSS nguồn cho SCORM player.
    "react-shim.ts", # File shim cần thiết cho việc build các component React.

    # --- 5. Các dịch vụ (services) được sử dụng khi "Export SCORM" được gọi từ platform ---
    "packages/interactive-quiz-kit/src/services/scormPackaging.ts",     # Dịch vụ chính để tạo file ZIP.
    "packages/interactive-quiz-kit/src/services/HTMLLauncherGenerator.ts", # Dịch vụ để tạo file index.html bên trong gói SCORM.
    "packages/interactive-quiz-kit/src/services/SCORMManifestGenerator.ts" # Dịch vụ để tạo file imsmanifest.xml.
]
...
edited_files = [
    # CSDL: Tạo trigger để tự động tạo bản ghi review trống
    # (SQL Script, không phải file trong repo)
    # CSDL: Cập nhật RPC get_live_session_updates để thêm trạng thái review
    # (SQL Script, không phải file trong repo)

    # Actions: Tạo các server action để xử lý logic review
    "src/lib/actions/reviewActions.ts",

    # Components: Component UI chính để hiển thị kết quả và AI review
    "src/components/teacher/live-session/LiveExamResultAnalysis.tsx",

    # Pages: Tích hợp component mới vào luồng của học sinh
    "src/app/live/[shortCode]/LiveExamClientFlow.tsx",

    # Components: Tích hợp logic và UI review vào dashboard của giáo viên
    "src/components/teacher/live-session/LiveSessionDashboard.tsx",

    # Actions: Cập nhật action nộp bài để trả về submissionId
    "src/lib/actions/liveExamActions.ts",
    
    # CSDL: Cập nhật RPC submit_live_exam_rpc để trả về submissionId
    # (SQL Script, không phải file trong repo)
]
...
class GitFileTracker:
    def __init__(self, project_path: str, output_dir: str = "tracked_files",
        ...
        # scan_dir: giới hạn quét chỉ trong thư mục con (vd "DemoApp").
        # None = toàn bộ repo. get_tracked_files/initial_scan/tree sẽ lọc theo prefix này.
        self.scan_dir: str = (scan_dir or "").strip().strip("/")

        # file_types: map loại file → extensions. Có thể truyền từ ngoài
        # (vd: {"swift": [".swift"], "python": [".py"], "c_cpp": [".c",".cpp",".ino"]})
        # thay vì hardcode chỉ TS/JS.
        if file_types is None:
            file_types = {
                'typescript': ['.ts', '.tsx'],
                'javascript': ['.js', '.jsx'],
            }
        self.file_types: Dict[str, List[str]] = file_types
    ...
    def get_tracked_files(self) -> List[str]:
        try:
            ...
            result_files = [f for f in files if f and not self.should_ignore_file(f)]
            # Giới hạn scan theo scan_dir (nếu có) — chỉ giữ file trong thư mục con
            if self.scan_dir:
                prefix = self.scan_dir.replace('\\', '/') + '/'
...
def main():
    ...
    parser.add_argument('--file-types', default=None, metavar='JSON',
                        help='(MỞ RỘNG) Map loại file → extensions, JSON. '
                             'VD: \'{"swift":[".swift"],"python":[".py"],"c_cpp":[".c",".cpp",".ino"]}\'. '
                             'Ghi đè file_types mặc định (typescript/javascript).')
    parser.add_argument('--scan-dir', default=None,
                        help='(MỞ RỘNG) Giới hạn scan/tree chỉ trong thư mục con của project '
                             '(VD: "DemoApp"). None = toàn bộ repo.')
    ...
    # Parse --file-types JSON (nếu có) — thay file_types mặc định
    file_types_override = None
    if args.file_types:
        try:
            file_types_override = json.loads(args.file_types)
            if not isinstance(file_types_override, dict):
                raise ValueError("--file-types phải là JSON object {type: [extensions]}")
        except Exception as e:
            parser.error(f"--file-types không hợp lệ: {e}")

    tracker = GitFileTracker(str(project_path_resolved), args.output_dir,
                             file_types=file_types_override, scan_dir=args.scan_dir)
