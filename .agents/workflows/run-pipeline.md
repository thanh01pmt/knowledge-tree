---
description: Thực thi toàn bộ quy trình Knowledge Tree End-to-End với 5 điểm kiểm duyệt của con người (HITL - Human-in-the-loop).
---

# Workflow: Run Full Pipeline (HITL)

> Trình tự chuẩn mực (Golden Path) để xây dựng một project Knowledge Tree hoàn chỉnh từ con số 0. Workflow này kết nối tất cả các tool và quy định rõ **5 Điểm Dừng (Checkpoints)** cần người dùng phê duyệt trước khi đi tiếp.
...
4. **Map Taxonomy (Dựa trên ATE Hints):**
   ```bash
   /map-taxonomy
   ```

> 🛑 **HITL Checkpoint 3: Phê duyệt Cấu trúc N:N**
...
5. **Lắp ráp Cây (Build Tree):**
   ```bash
   /build-tree
   ```
   *(Tạo ra 5 TSV: fields, subjects, categories, topics, concepts).*

---

## 🧠 Giai đoạn 3: Hierarchical Learning Objectives & Curriculum DAG

6. **Sinh Universal LOs (Phase A):**
   ```bash
   /generate-ulos
   ```

> 🛑 **HITL Checkpoint 4: Phê duyệt ULO (Tính Độc Lập)**
...
7. **Sinh Conceptual LOs (Phase B - Phép thử Marr):**
   ```bash
   /generate-cios
   ```

> 🛑 **HITL Checkpoint 5: Nghiệm thu Phép thử Marr**
...
8. **Sinh Specific LOs & Nối Dây Tiền đề (Phase C, D, E):**
   ```bash
   /generate-sios
...
9. **Kiểm thử (Validation & Audit):**
   ```bash
   /validate-tree
   /audit-coverage
   /detect-gaps
   ```
   *(Phải PASS 100% không có lỗi tham chiếu).*

10. **Đồng bộ Lên Cloud (Supabase):**
    ```bash
    /sync-supabase
    ```
    *(Đồng bộ 7 bảng theo đúng thứ tự Top-down, không bẻ gãy khóa ngoại).*
