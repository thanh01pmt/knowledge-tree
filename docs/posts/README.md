# Series Bài Viết Xu Hướng Công Nghệ & Giáo Dục Khoa Học Máy Tính (2026)

Chào mừng bạn đến với bộ sưu tập các bài viết phân tích chuyên sâu về **Xu hướng Công nghệ Kỹ thuật & Đổi mới Giáo dục Khoa học Máy tính (2025–2026)** do **Antigravity AI Team** nghiên cứu và tổng hợp từ các nguồn hàn lâm quốc tế (ACM/IEEE CS2023, Stanford/MIT/CMU Curricula, NIST, TinyML Foundation, arXiv).

---

## 📚 Danh Mục Các Bài Viết Trong Series

### 1. [4 Dịch Chuyển Paradigm Trong Giáo Dục Khoa Học Máy Tính (2025–2026)](./xu_huong_giao_duc_cs_2025_2026.md)
* **Chủ đề:** Đổi mới chương trình đào tạo CS, Competency-Driven Verification, Green AI, AI Governance, Probabilistic Reasoning.
* **Tóm tắt:** Phân tích bước chuyển từ việc dạy lập trình gõ code cơ học sang năng lực xác minh (AI Auditor), điện toán xanh tiết kiệm năng lượng và nhúng đạo đức vào quy trình kỹ thuật.
* **Link:** [`xu_huong_giao_duc_cs_2025_2026.md`](./xu_huong_giao_duc_cs_2025_2026.md)

---

### 2. [Kỷ Nguyên Agent Harness Engineering & Model Context Protocol (MCP)](./agentic_ai_harness_and_mcp_2026.md)
* **Chủ đề:** Agentic AI, Chuẩn MCP, Agent Harness, PRAR Loop, State & Memory Management.
* **Tóm tắt:** Phân tích sự chuyển dịch từ thiết kế tập trung vào Model (Model-Centric) sang thiết kế Hạ tầng Agent Harness bền vững, chuẩn hóa giao thức MCP và điều phối đa tác nhân.
* **Link:** [`agentic_ai_harness_and_mcp_2026.md`](./agentic_ai_harness_and_mcp_2026.md)

---

### 3. [Thế Hệ RAG Mới: GraphRAG, Hybrid Search & Retrieval Engineering](./advanced_rag_graphrag_vector_search_2026.md)
* **Chủ đề:** Advanced RAG, GraphRAG, Hybrid Search (Vector + Sparse BM25), Vector in RDBMS, Khung RAGAS.
* **Tóm tắt:** Lộ trình nâng cấp từ Naive RAG sang Modular RAG, giải quyết suy luận đa bước bằng Đồ thị Tri thức (GraphRAG) và kết hợp Vector Extension trực tiếp trong SQL Database.
* **Link:** [`advanced_rag_graphrag_vector_search_2026.md`](./advanced_rag_graphrag_vector_search_2026.md)

---

### 4. [AIoT Và TinyMLOps: Đưa Mô Hình Ngôn Ngữ Nhỏ (SLM) Lên Thiết Bị Biên](./tinymlops_edge_ai_aiot_2026.md)
* **Chủ đề:** TinyML, Edge AI, AIoT, Compute-in-Memory (CIM), TinyMLOps, Federated Learning.
* **Tóm tắt:** Đột phá phần cứng CIM giúp chạy suy luận mô hình học máy và SLM trên vi điều khiển công suất cực thấp, cắt giảm 95% băng thông và 70% chi phí vận hành.
* **Link:** [`tinymlops_edge_ai_aiot_2026.md`](./tinymlops_edge_ai_aiot_2026.md)

---

### 5. [Kiến Trúc An Ninh Mạng Zero Trust & Chuẩn Bị Cho Kỷ Nguyên Lượng Tử](./cybersecurity_zero_trust_quantum_2026.md)
* **Chủ đề:** Cybersecurity, Zero Trust Architecture, IAM, PKI, Threat Modeling (STRIDE), Post-Quantum Cryptography (PQC).
* **Tóm tắt:** Phân tích nguyên tắc an ninh Zero Trust ("Never Trust, Always Verify"), quy trình mô hình hóa mối đe dọa STRIDE và sự chuẩn bị hạ tầng mã hóa kháng lượng tử (PQC).
* **Link:** [`cybersecurity_zero_trust_quantum_2026.md`](./cybersecurity_zero_trust_quantum_2026.md)

---

### 6. [Mô Hình Tiến Trình Xoắn Ốc (Bruner's Spiral Curriculum) Trong Tin Học Phổ Thông](./bruner_spiral_curriculum_k12_preuniv_cs.md)
* **Chủ đề:** Bruner Spiral Curriculum, CSTA K-12 CS Standards, AP CSP, IB CS, CS Unplugged.
* **Tóm tắt:** Phương pháp thiết kế chương trình tin học xoắn ốc từ Phổ thông đến Tiền Đại học, giúp học sinh tiếp cận một khái niệm cốt lõi ở nhiều cấp độ nhận thức tăng dần mà không bị "khóa tuổi".
* **Link:** [`bruner_spiral_curriculum_k12_preuniv_cs.md`](./bruner_spiral_curriculum_k12_preuniv_cs.md)
---

### 7. [Khoa Học Máy Tính Liên Môn: Nhánh S-T-E-M Tích Hợp Cho Kỷ Nguyên Tính Toán Đa Ngành](./stem_interdisciplinary_cs_integration_2026.md)
* **Chủ đề:** STEM Interdisciplinary, Computational Science, Mechatronics, Signal Processing (DSP), Information Theory, Post-Quantum DLT.
* **Tóm tắt:** Phân tích vai trò của CS như một "Trục Điện toán Trung tâm" kết nối Sinh học Tính toán (Bioinformatics), CFD Vật lý, Điện toán Không gian XR, Cơ điện tử HIL, Xử lý Tín hiệu Số DSP và Đại số Tensor SVD.
* **Link:** [`stem_interdisciplinary_cs_integration_2026.md`](./stem_interdisciplinary_cs_integration_2026.md)
---

### 8. [Khung Giáo Dục STEM K-12 Quốc Tế Và Chương Trình GDPT 2018](./k12_stem_frameworks_ngss_gdpt2018.md)
* **Chủ đề:** K-12 STEM Education, NGSS 3D Learning, CSTA K-12, GDPT 2018 Việt Nam, Engineering Design Cycle.
* **Tóm tắt:** Phân tích cấu trúc khung chuẩn STEM NGSS 3D Learning, sự tích hợp giữa NGSS SEPs và CSTA K-12, hai quy trình Khám phá Khoa học / Thiết kế Kỹ thuật và đối chiếu với 3 hình thức STEM trong GDPT 2018 Việt Nam.
* **Link:** [`k12_stem_frameworks_ngss_gdpt2018.md`](./k12_stem_frameworks_ngss_gdpt2018.md)
---

### 9. [Giao Thoa STEAM: Chữ 'A' (Arts) Trong Đồ Họa, Tư Duy Thiết Kế Và Nghệ Thuật Thuật Toán](./steam_arts_design_thinking_generative_art.md)
* **Chủ đề:** STEAM Education, Arts in STEM, Design Thinking, Generative Art, Creative Coding, WCAG Accessibility.
* **Tóm tắt:** Phân tích vị trí của nhánh Arts trong mô hình STEAM, bao gồm 5 bước Tư duy Thiết kế (Design Thinking HCD), Thẩm mỹ Đồ họa (Color/Typography/Grid), Nghệ thuật Thuật toán (Generative Art / Perlin Noise) và Thiết kế Tiếp cận Hòa nhập (Inclusive UX / WCAG).
---

### 10. [Các Biên Giới Mới Trong Khoa Học Máy Tính: Điện Toán Mô Phỏng Sinh Học, Kiểm Chứng Hình Thức Và Hệ Thống Tự Chủ](./frontier_computer_science_domains_2026.md)
* **Chủ đề:** Bio-Inspired Computing, Neuromorphic Chips, Formal Verification, Speech & NLP, Autonomous Drones, Sensor Fusion.
* **Tóm tắt:** Phân tích 4 biên giới mới của CS năm 2026: Chip nơ-ron vi mạch (Spiking Neurons SNNs), Kiểm chứng phần mềm hình thức (Model Checking & Type Safety), Xử lý Tiếng nói STT/TTS & Cú pháp NLP, cùng Hệ thống Tự chủ Tự điều hướng Hợp nhất Đa Cảm biến (LiDAR/Kalman Filter).
* **Link:** [`frontier_computer_science_domains_2026.md`](./frontier_computer_science_domains_2026.md)

---
Tất cả các bài viết trong series đều đi kèm **Sơ đồ Mermaid rendered trực quan**, **Mã nguồn/Khái niệm 100% Trung tính T6** và **Link dẫn chứng tham chiếu từ các tổ chức hàn lâm uy tín** (ACM, IEEE, NIST, Stanford, MIT, CSTA, College Board).

Mọi góp ý và thảo luận xin vui lòng gửi về repository chính thức của dự án **Knowledge Tree**.
