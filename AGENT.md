...
- **Định dạng mã SIO**: `SIO-<TECH_PREFIX>-<FEATURE_SLUG>` (ví dụ: `SIO-JS-OBJECT-LITERAL-METHODS`, `SIO-SWIFT-OBJECT-LITERAL-METHODS`, `SIO-PY-DICT-KEYS-VALUES-GET`).
- **Đối chiếu Mẫu SIO Đa Công nghệ (Cross-Technology Pattern Reference)**: 
  - Do tầng Concept, ULO và CIO là **100% Trung tính** và dùng chung cho mọi ngôn ngữ, khi xây dựng SIO cho một công nghệ mới (ví dụ: Swift), Agent **CẦN TRA CỨU & ĐỐI CHIẾU** các SIO đã có ở các cây công nghệ khác (như JS, Python) nằm cùng mã CIO/Concept để nhân bản mẫu mã và đổi tên/nội dung tương đương cho công nghệ mới.
...
- Mọi dự án phải trích xuất cạn kệt 100% các chủ đề, công cụ, thư viện trong `context/*.json` và `context/*.pdf` với quy mô trung bình **$\ge 80 - 160$ LOs**.
...
### 6. Cấm Sử dụng Script Thế Chuỗi Tự động Thô ráp (No Dumb Find-and-Replace)
...
### 7. Không Tạo Script Rác ở Thư mục Root
