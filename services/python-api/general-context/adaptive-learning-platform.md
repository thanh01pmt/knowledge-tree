...
* **Phiên bản:** 2.3 (Bản Cập nhật Kiến trúc Multi-MCP Hub, Prerequisite DAG & Visual Graph Explorer)
...
#### **2.4. Mô hình Đồ thị Tiền đề (Prerequisite DAG) & Thứ tự Tuyến tính (Phiên bản v2.3)**

**Cơ sở Dữ liệu Thực tế trong Supabase Cloud:**
Để biến mô hình cây phân cấp thành một **Đồ thị Hướng Không Chu trình (Directed Acyclic Graph - DAG)** phục vụ cho thuật toán tìm đường đi ngắn nhất (Shortest Path DAG), hệ thống v2.3 chính thức chuẩn hóa 3 bảng dữ liệu cốt lõi trong Supabase DB:

1. **Bảng Quan hệ Tiền đề: `learning_objective_prerequisites`**
   - Cấu trúc: `learning_objective_code` (Target LO) $\leftrightarrow$ `prerequisite_lo_code` (LO Tiền đề bắt buộc).
   - Vai trò: Khai báo chính thức mối quan hệ phụ thuộc trước-sau giữa các LOs (ví dụ: `LO-GQL-CACHE` phụ thuộc `LO-GQL-EXEC-RESOLVER`).

2. **Cột Thứ tự Sư phạm: `sequence_order` Trên Các Bảng Junction**
   - Tất cả các bảng nối N:N (`field_subjects`, `subject_categories`, `category_topics`, `topic_concepts`, `concept_learning_objectives`) đều được bổ sung cột `sequence_order` (kiểu số nguyên).
   - Vai trò: Duy trì thứ tự giảng dạy/học tập chuẩn hóa do chuyên gia sư phạm thiết lập.

3. **Bảng Hồ sơ Năng lực Cá nhân hóa: `student_mastery`**
   - Cấu trúc: `student_id`, `learning_objective_code`, `mastery_level` (`not_started`, `in_progress`, `mastered`), `mastery_score`, `confidence_score`, `attempt_count`, `correct_attempts`.
   - Vai trò: Cung cấp trạng thái năng lực thực tế của học viên để bộ thuật toán **Learning Path Navigator** tự động bỏ qua các LOs đã đạt (`mastered`) và chỉ vẽ đường đi chứa các LOs còn hổng.
...
#### **4.4. Hạ tầng Multi-MCP Hub & Visual Knowledge Tree Explorer (Phiên bản v2.3)**

##### **1. Kiến trúc Multi-MCP Hub Server (`https://kt-mcp.orchable.xyz/mcp`)**
Để giúp các AI Coding Agent (Cursor, Claude Desktop, Antigravity) và các dịch vụ bên ngoài truy vấn trực tiếp vào Cây tri thức và các công cụ sư phạm, hệ thống triển khai hạ tầng **Multi-MCP Hub Server** dựa trên FastMCP v3:
- **Tên miền Công khai chính thức**: `https://kt-mcp.orchable.xyz/mcp` (Health check: `https://kt-mcp.orchable.xyz/health`).
- **Cơ chế Server Composition (`mount()`)**: Hub chính tích hợp các Sub-Servers chuyên biệt qua namespace:
  - `kt_*`: Quản trị và kiểm tra tính toàn vẹn Cây tri thức (`kt_validate_tree`, `kt_detect_gaps`, `kt_audit_coverage`, `kt_sync_supabase`).
  - `sys_*`: Quản lý tài nguyên hệ thống và cung cấp prompt sư phạm (`sys_get_system_status`, `skills://{name}`).
- **Triển khai Cloud & CI/CD**: Chạy trong Docker Container trên máy chủ Oracle Cloud VM với pipeline tự động hóa GitHub Actions (kích hoạt khi push/merge vào nhánh `stable`).

##### **2. Giao diện Trực quan & Lộ trình Học Tối ưu (Visual Graph & Path Navigator)**
Nhằm trực quan hóa đồ thị tri thức 6 tầng cho người học và giáo viên, v2.3 giới thiệu giao diện **Visual Knowledge Tree Explorer**:
- **Multi-View System**:
  - *Graph View (DAG)*: Trực quan hóa đồ thị phân cấp N:N với hiệu ứng tự động Highlight chuỗi Tri thức Tiền đề (Prerequisite Chain).
  - *Pedagogy Matrix View*: Ma trận 2 chiều (6 cấp độ Bloom x 4 Knowledge Dimensions) với Heatmap màu cảnh báo vùng hổng tri thức.
  - *Learning Path Navigator*: Thuật toán **Shortest Path DAG** kết hợp giữa `learning_objective_prerequisites`, `sequence_order` và `student_mastery` để vẽ ra con đường ngắn nhất duy nhất loại bỏ các tri thức dư thừa.
- **FastMCP Prefab UI**: Tận dụng tính năng Generative UI của FastMCP v3 (`@mcp.tool(app=True)`) cho phép nhúng trực tiếp giao diện đồ thị tương tác vào cửa sổ Chat UI của AI Agent.
